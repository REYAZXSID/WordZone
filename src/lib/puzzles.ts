...
const getDifficulty = (quote: string): Difficulty => {
    ...
    if (length >= 30 && length <= 35) {
        // To differentiate, we can alternate between hard and intermediate for this range
        // This is a simple heuristic. A better way might be to add a manual flag.
        return length % 2 === 0 ? 'hard' : 'intermediate';
    }
    if (length > 35 && length <= 39) return 'advance';

    // Fallback for quotes outside the new ranges
    if (length > 39) return 'advance';
